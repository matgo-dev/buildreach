...
所有业务状态字段的变更必须走状态机/状态流转服务,不允许在路由或普通 CRUD 逻辑里直接执行 `update(status=...)` 或随意赋值绕过流转规则。

**必须遵守**:
- 每个有业务含义的状态字段都要定义合法状态集合和合法流转关系
- 状态变更 API 应优先设计为动作接口(如 publish / unpublish / submit / approve / reject / cancel),不要暴露通用 `status` 任意更新
- 状态机负责校验当前状态、目标状态、业务前置条件、操作者权限和审计记录
- 批量状态变更也必须复用同一套状态机校验,不得单独写批量 SQL 直接改状态
- 初始创建状态可以由创建流程赋默认值,但创建后的任何状态流转都必须进入状态机

**绝对禁止**:
- ❌ 在普通 update schema 中开放业务 `status` 任意修改
- ❌ 使用 SQLAlchemy `update(Model).values(status=...)` 绕过状态机
- ❌ 在不同服务里复制粘贴一套状态判断,导致规则分叉
