...
| AI | 通义千问 `qwen-plus`（DashScope OpenAI 兼容端点）+ Tavily 联网检索 |
...
| 模块 | 说明 |
|------|------|
| **商品目录** | SPU+SKU 两层、i18n 分列、阶梯价、多图、供货关系、商品/SKU 双维属性 |
...
| 生产 | OVHcloud（海外） | GHCR | 支持离线包 + OpenResty 部署 |
...
| 演进项 | 触发条件 | 说明 |
|--------|----------|------|
| **对象存储 (OVH S3)** | 近期 | 商品/附件图片从 Docker 卷迁至 OVH S3。`services/storage.py` 已是 Protocol 抽象，契约见 `docs/xfs_s3_image_upload_contract.md`；顺带用生命周期规则清理孤儿附件 |
| **Redis** | 后端需多实例时 | 多实例后内存态限流会被绕过、各实例缓存不同步、调度重复触发。届时用 Redis 承接分布式限流、共享缓存（品类/术语/首页楼层）、以及 AI agent 的任务队列与结果缓存 |
| **Kubernetes** | 单机资源见顶或需高可用/多副本弹性时 | 现阶段 Docker Compose 单机足够；当出现单点可用性要求、需按流量自动扩缩容、或多服务编排复杂度上升时再引入。前置条件是先完成 Redis 外部化与 S3 迁移，使服务真正无状态 |

## 开发约定

- **严禁直接在 `main` 上 commit / push**，一律走分支 + PR（见 `AGENTS.md`）。
- UI 遵循 `DESIGN.md` 双轨设计系统（买方前台 Teal / 运营后台 Brand Blue，严禁混用）。
- 安全与性能为当前保障重点；引入 AI agent 时复用 `services/llm/`（可切换国产模型）与 `services/credit/` 现有服务层。
