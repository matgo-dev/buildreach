...
| 演进项 | 触发条件 | 要点 |
|--------|----------|------|
| **邮件中继 (SMTP→中继/API)** | **正式上线前(阻塞项)** | 直发 SMTP 会在邮件头暴露源站 IP；上线前须改用第三方中继 / 邮件 API（Resend / SES 等），见 `email_service.py` 的 `TODO(T-EMAIL-API)`。中继就绪前，买方注册的邮箱验证可用 `REQUIRE_EMAIL_VERIFICATION=false` 临时关闭 |
| **对象存储 (OVH S3)** | 近期 | 图片从 Docker 卷迁至 S3；`services/storage.py` 已是 Protocol 抽象，契约文档在本地 `docs/`（未纳入版本库） |
| **托管数据库 (OVH Managed PostgreSQL)** | 正式接客户前 | 现为容器单机 `db`(pg 16.4)，部署前 pg_dump 本机备份仅供回滚、非灾备。接客户后「数据不能丢 + 不能停机」→ 迁 OVH 托管 **Business 计划(2 节点/高可用)**，起步 **DB1-4**(4GB/1 核，约 $164/月)，自带自动备份 + PITR。迁移低风险(仅用 `pg_trgm`，OVH 标配)：开实例 → `pg_dump` → 导入 → 改 `DATABASE_URL` → 删 compose `db`。跳过 Essential(单节点无 HA、两头不占) 与 Enterprise(体量用不到) |
