...
session-scope seed 已建评分骨架；用 db_session 直接调 run_harvest_for_company。
...
async def test_harvest_writes_run_snapshots_and_scores(client, db_session):
    ...
    run = await run_harvest_for_company(
        db_session, cid, triggered_by="manual", operator_user_id=None, harvester=fake
    )
...
async def test_register_trigger_uses_real_time_onboard(client, db_session):
    ...
    await run_harvest_for_company(
        db_session, company.id, triggered_by="supplier_register", harvester=fake
    )
    await db_session.commit()
    snap = (await db_session.execute(
        select(ScoreSnapshot).where(
