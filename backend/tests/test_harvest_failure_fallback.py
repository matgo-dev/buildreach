...
async def test_all_dimensions_failed_run_failed_but_scores(client, db_session):
    cid = await _new_kh(db_session, "KHF1")
    fake = FakeHarvester(
    ...
    run = await run_harvest_for_company(db_session, cid, "manual", harvester=fake)
    await db_session.commit()

    assert run.status == "failed"
    assert run.error_detail  # 汇总各维度错误
    # 失败维度不落快照
    basic = (await db_session.execute(
...
async def test_partial_failure_run_partial_succeeded(client, db_session):
    cid = await _new_kh(db_session, "KHF2")
    fake = FakeHarvester(
...
async def test_qualification_missing_clears_stale_mock_certs(client, db_session):
    """回归:真实抓取 qualification=missing 时清掉 Δ5 mock 证书,不让残留充数评分。"""
    cid = await _new_kh(db_session, "KHF3")
    _add_mock_cert(db_session, cid)
    ...
    await run_harvest_for_company(db_session, cid, "manual", harvester=fake)
    await db_session.commit()
...
async def test_qualification_failed_keeps_certs(client, db_session):
    """qualification=failed(抓取出错)时保留旧证书,不能凭一次失败断言无资质。"""
    cid = await _new_kh(db_session, "KHF4")
    _add_mock_cert(db_session, cid)
    await db_session.flush()
    fake = FakeHarvester(
        basic=make_result("ok", "public", FULL_BASIC),
