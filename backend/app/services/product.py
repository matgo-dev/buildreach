...
async def _generate_spu_code(db: AsyncSession, category_code: str) -> str:
    """根据品类编码自动生成 SPU 编码，格式：{品类前缀}-{品类层级}-{序号}。"""
...
async def _generate_sku_code(
    ...
    """基于 SPU 编码生成 SKU 编码，格式：{spu_code}-S{序号}。"""
    count_result = await db.execute(
...
async def create_product(
    ...
    product = Product(
        ...
        selling_points_i18n=data.selling_points_i18n,
...
async def list_products_operator(
    ...
    *,
    ...
    if keyword:
        kw = f"%{keyword}%"
        q = q.where(Product.name.ilike(kw) | Product.spu_code.ilike(kw))
        count_q = count_q.where(Product.name.ilike(kw) | Product.spu_code.ilike(kw))
...
async def list_products_public(
    ...
    *,
    ...
    """公开列表：只返回 ACTIVE SPU，加载 SKU（不加载供货关系）。"""
    q = select(Product).options(
        selectinload(Product.images),
...
async def create_sku(
    ...
    sku = ProductSku(
        ...
        name_i18n=data.name_i18n,
        color=data.color,
        color_i18n=data.color_i18n,
        material=data.material,
        material_i18n=data.material_i18n,
...
async def update_sku(
    ...
    for field, value in data.model_dump(
        exclude_unset=True, exclude={"price_tiers"},
    ).items():
...
def _validate_price_tiers(tiers: list[PriceTierCreate], moq: int) -> None:
    """校验阶梯价规则：首档 min_qty = moq，连续，递减，末档 max_qty 可空。"""
    if not tiers:
        return
