...
# ── 九大 L1 品类 ──────────────────────────────────────────────
L1_CATEGORIES = [
    ("01", "照明", "Lighting", "lightbulb", 10),
    ("02", "电气", "Electrical", "zap", 20),
    ("03", "卫浴洁具", "Sanitary & Bath", "droplets", 30),
    ("04", "五金工具", "Tools & Hardware", "wrench", 40),
    ("05", "板材", "Boards & Panels", "layers", 50),
    ("06", "管件", "Pipes & Fittings", "cylinder", 60),
    ("07", "吊顶装饰", "Ceiling & Decoration", "palette", 70),
    ("08", "劳保用品", "Safety & PPE", "hard-hat", 80),
    ("09", "结构建材", "Structural & Chemical", "building", 90),
]
# 格式: (code, name_zh, name_en, icon, sort_order)
...
# 格式: (code, name_zh, name_en, sort_order)
...
async def seed_categories(db: AsyncSession) -> None:
    ...
    # L1
    for code, name_zh, name_en, icon, sort_order in L1_CATEGORIES:
        if await _upsert_category(
            db,
            code=code,
            name_zh=name_zh,
            name_en=name_en,
            level=CategoryLevel.L1,
            icon=icon,
            sort_order=sort_order,
        ):
            created += 1
