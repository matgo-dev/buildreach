...
连接中国建筑工业品供应商与东非本地建材买家的 B2B 数字供应链平台。平台作为唯一中转方，买卖双方互不可见（断层隔离）。

平台核心能力:
1. **建材商城** —— 面向东非买方的 B2B 电商门户,9 大品类、品类导航、询价报价
2. **运营管控后台** —— 商品上架(N:M 供货关系)、供应商管理、订单履约、拼箱集运
3. **AI 智能体工具箱** —— 占位阶段(标准问答 / 证书审查 / 报价比价 / 多语种翻译)
4. **履约风控中枢** —— 11 节点订单履约追踪 + 风控驾驶舱(后续 Phase)

**当前阶段**:
- ✅ 已完成：认证、RBAC、审计底座、信用评估
- ✅ 已完成：商品目录（products + product_suppliers N:M + 品类属性 EAV + 图片）
- 🚧 进行中：前端 1:1 复刻东非 Demo 截图
- ⏳ 待做：询价报价(Phase 2)、订单履约(Phase 3)、拼箱集运(Phase 4)
- ❌ 当前数据库**没有** Order / RFQ / Quote / Payment / Container 相关表（需设计对齐后再建）
...
| `docs/east-Africa/东非平台_MVP_PRD_v1.0.md` | 东非平台 MVP PRD（仅供参考，不完全执行） | 涉及东非业务功能时 |
| `docs/east-Africa/东非建材数字供应链平台管理后台Demo_11-20_高清PNG/` | **前端视觉权威来源**（买方商城 + 运营后台截图） | **所有前端开发时必看** |
| `docs/prompts/prompt-ea-*.md` | 东非平台各轮实施 prompt | 该轮实施时 |
...
#### i18n 多语言约定（强制，详见 `docs/architecture/i18n_技术方案_v2.md`）
