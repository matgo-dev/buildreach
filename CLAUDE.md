...
所有表使用 `SoftDeleteMixin`，提供以下字段：

| 字段 | 类型 | 说明 |
|---|---|---|
| `deleted_at` | `DateTime, nullable` | 删除时间,NULL = 未删除 |
| `deleted_by` | `Integer, FK → users.id, nullable` | 操作人 |

**写入规则**：
- 删除操作：`obj.deleted_at = utcnow(); obj.deleted_by = current_user.id`，**绝不** `db.delete()` / `DELETE FROM`
- 级联软删：删 SPU 必须同步软删其下所有 SKU、图片记录、属性、价格梯度、供应商关系等子实体，不留孤儿
- 恢复操作：置空 `deleted_at` / `deleted_by`，需权限控制
- 删除前置条件：`ACTIVE` 状态的商品必须先下架再删除；SKU 全部软删后，SPU 自动下架

**查询规则**：
- 所有常规查询必须过滤 `WHERE deleted_at IS NULL`，Service 层统一封装
- Buyer API 必须过滤，无例外
- Operator 默认过滤，后续可加「已删除」筛选视图
- 已删除记录不可上架、不可编辑、不可参与新业务

**唯一约束**：
- 带唯一约束的字段（如 SKU code）必须使用 **partial unique index**（`WHERE deleted_at IS NULL`），允许软删后重建同名记录

**二、文件资源（图片等）— 延迟物理清理**

图片删除 ≠ 立即删文件。DB 事务和文件系统不是一个事务，先删文件库失败会丢文件，先写库再删文件失败会残留。
...
**绝对禁止**：
- ❌ 业务表 `db.delete()` / `DELETE FROM` / `sa_delete()`
- ❌ 查询不带 `deleted_at IS NULL` 过滤
- ❌ 父实体软删但子实体仍为活跃状态（孤儿数据）
- ❌ 软删字段手动写，不走 `SoftDeleteMixin`
- ❌ 在请求链路中同步删除文件
