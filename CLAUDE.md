...
核心原则：**软删保护"业务对象"，不是保护每一行明细数据**。属性、阶梯价这种"当前配置明细"，靠父级软删 + 审计/订单快照就够了。

**一、软删除 — 有独立业务身份的实体**

以下实体使用 `SoftDeleteMixin`（`deleted_at` + `deleted_by`），绝不 `db.delete()`：

| 实体 | 理由 |
|---|---|
| `Product` (SPU) | 被订单/询价引用，需审计追溯 |
| `ProductSku` (SKU) | 被订单行项引用 |
| `ProductSupplier` (供货关系) | 有供应商、成本、认证、优选等业务含义 |
| `ProductImage` (图片 DB 记录) | 配合文件延迟清理，订单快照可能引用 |

**软删写入规则**：
- 删除操作：`obj.deleted_at = utcnow(); obj.deleted_by = current_user.id`
- 级联软删：删 SPU 必须同步软删其下 SKU、图片记录、供货关系
- 恢复操作：置空 `deleted_at` / `deleted_by`，需权限控制
- 删除前置条件：`ACTIVE` 商品必须先下架再删除；SKU 全部软删后 SPU 自动下架

**软删查询规则**：
- 所有常规查询必须过滤 `WHERE deleted_at IS NULL`
- Buyer API 必须过滤，无例外
- Operator 默认过滤，后续可加「已删除」筛选视图
- 已删除记录不可上架、不可编辑、不可参与新业务

**唯一约束**：
- 软删实体的唯一字段使用 **partial unique index**（`WHERE deleted_at IS NULL`），允许软删后重建同名记录

**二、硬删除 — 附属配置数据 / 系统内部数据**

以下实体直接 `db.delete()` / `sa_delete()`，不走 `SoftDeleteMixin`：

| 实体 | 理由 |
|---|---|
| `ProductAttr` (属性) | 随父级有效性过滤，编辑时全量替换，无独立业务含义 |
| `SkuPriceTier` (阶梯价) | 随 SKU 有效性过滤，编辑时全量替换 |
| `Permission` (权限) | 系统配置同步，非用户业务行为 |
| `RolePermission` (角色权限) | 系统配置镜像同步 |
| `CreditSearchHistory` (搜索历史) | 用户隐私清理操作，审计价值低 |

**三、文件资源（图片等）— 延迟物理清理**

图片删除 ≠ 立即删文件。DB 事务和文件系统不是一个事务。

删除流程：
1. DB：`product_images.deleted_at = now()`，记录软删
2. 前台/后台查询默认不再展示
3. 文件：不在请求链路里立即删，进入待清理队列
4. 清理任务确认无活跃引用 + 超过保留期后，物理删除文件

保留期（MVP）：
- 被替换/删除的商品图片：**30 天**后清理文件
- 被软删商品/SKU 下的图片：**90 天**后清理文件
- 已被订单/询价快照引用的图片：**不清理**，或复制为单据快照资源后再清理原图

**四、单据快照（强制）**

历史订单/询价**不得依赖当前商品图片**。单据生成时必须快照：商品名、SKU 名、规格、价格、主图 URL/快照 key。商品图片后续删改不影响历史单据展示。

**绝对禁止**：
- ❌ 软删实体（SPU/SKU/供货关系/图片记录）用 `db.delete()`
- ❌ 软删实体查询不带 `deleted_at IS NULL` 过滤
- ❌ 父实体软删但子实体仍为活跃状态（孤儿数据）
- ❌ 在请求链路中同步删除文件
- ❌ 历史单据直接引用当前商品图片 URL（必须快照）
...
- **操作列禁止换行** — 所有表格/列表的"操作"列必须 `whitespace-nowrap`，列宽用 `auto` 或足够宽的固定值，按钮不允许折行
- **买方前台按钮要精致** — 买方可见的页面（buyer workspace）按钮用圆角 pill 样式（`rounded-full` + `border` + `shadow-sm`），不用裸文本链接风格；后台（operator/admin）可以用简洁文本按钮
- **多语言 UI 适配** — 所有 UI 改动必须同时验证中/英/斯瓦希里三种语言下的布局。英文和斯瓦希里文案通常比中文长，按钮/列宽/布局必须留够空间，不能只按中文宽度设计导致其他语言溢出或错位
- **改一处改一类** — 优化某个页面的样式/交互后，必须检查同类页面是否有相同问题并一并修复。不能只改一个页面，留其他页面继续丑着
- **兄弟页面布局统一** — 共享同一 Tab/导航的兄弟页面，布局结构必须一致（卡片层级、间距、标题位置、筛选栏风格）。这是基本设计常识，不需要被提醒
...
### 出方案 / 动手前强制检查（无例外，每次都做）

**任何设计、方案、代码改动之前，必须先完成以下检查，缺一不可：**

1. **查记忆** — 读 `MEMORY.md` 索引，打开相关记忆文件，确认是否有**已确认的设计决策**。有决策的 → 直接按决策执行，不重新发明
2. **查阿里国际站** — 同类功能在 alibaba.com 怎么做的（信息架构、数据模型、交互逻辑）。这是首选参考源，不是可选项
3. **对比其他平台** — Shopify / Saleor / Magento 作为补充参考，给对比表 + 推荐
4. **启 Agent / Worktree 前** — 必须把上面 1-3 的结论写进 Agent prompt，不能让 Agent 自己从零摸索

**违反后果**：用户已多次纠正此问题。忘查记忆 = 重复讨论 = 浪费时间。

### 给了参考页面 / "按照XX那样做"时（强制）

**收到"参考XX页面"或"按照XX那种交互"指令时，必须先完整对照参考页面再动手写代码：**

1. **读完参考页面代码** — 逐项提取：每个列名、表头文案、按钮文字、链接文字、交互方式、颜色风格
2. **列对照清单** — 新页面 vs 参考页面，逐项对比，找出所有差异点
3. **主动对齐** — 能统一的文案、i18n key、布局结构，必须直接对齐，不自己发明新文案
4. **差异需要确认的先问** — 两个页面场景不同导致必须有差异的（如报价页多了单价/阶梯价列），标注出来

**绝对禁止**：
- ❌ 参考页面用"修改规格"，新页面自己写"编辑变体" — 文案必须统一
- ❌ 参考页面用"添加商品行"，新页面自己写"添加行项" — 同一概念同一文案
- ❌ 参考页面商品名+规格在同一列，新页面拆成两列 — 布局必须对齐
- ❌ 写完才对照，等用户逐项纠正 — 对照在前，编码在后
...
### 代码完成后强制自测三项（每次提交前，缺一不可）

**① UT（单元测试）**:
- `pytest tests/test_xxx.py` 跑通相关测试
- 已有测试且通过的可跳过，新增/修改逻辑必须补测试

**② 启动后端 + API 实测**:
- `uvicorn app.main:app --reload` 启动服务
- 用 curl / httpx **造真实数据**跑全流程走查（写入 → 异步处理 → 读取）
- 不能只验单步语法，必须跑完整条链路

**③ 前端自检**:
- UI 渲染是否正常（按钮/链接/颜色/布局是否可见、可点击）
- React Hooks 规则（无条件调用，不在循环/条件里调 hook）
- z-index 层级（品类 flyout、模态框、下拉菜单是否被遮挡 — **已反复出问题多次**）
- Tailwind 自定义色值是否在 `tailwind.config.ts` 中存在

**不自测就提交 = 必然漏运行时错误。用户不应该是第一个发现 bug 的人。**
...
6. **文件上传必须前端预校验** — 后端有的校验规则，前端必须同步做一遍，选文件时就拦截并告知具体原因，不能等提交后才报错。上传区域必须显示限制说明。
7. **后端错误必须透传** — `catch` 块禁止硬编码通用错误吞掉后端 `message` / `message_key`，必须展示具体原因。浏览器原生错误要翻译为友好提示。
8. **失败状态必须可恢复** — 失败的操作不能残留在前端状态导致死循环；多步提交中部分成功后 retry 必须跳过已完成步骤，不能重复创建资源。
9. **通知统一用 Toast** — 成功/警告/错误统一用 `useToast()`，禁止 `alert()` 或不显眼的行内文字。

---

## 踩坑教训汇总（从历次纠正中总结，强制遵守）

以下规则均来自实际踩坑后用户纠正，不是建议而是**必须遵守的红线**。

### Git 工作流

- **禁止直接在 dev / main 上 commit** — 哪怕一行 typo 也必须切分支走 PR，无例外（用户已强调 100+ 次）
- **切分支前必须问用户确认** — 用户经常多窗口工作，私自切分支会搞乱其他窗口
- **PR 合并后不删分支** — `gh pr merge` 不加 `--delete-branch`，保留分支历史
- **Commit 不加 Co-Authored-By** — 避免 GitHub Contributors 列表出现 Claude
- **Worktree 必须基于最新 dev** — 启 agent worktree 前必须 `git fetch origin dev && git checkout -b xxx origin/dev`，防止基线过旧导致代码白改

### 前端开发

- **页面内容必须铺满全屏** — `max-w-mall` 保持 `100%`，**绝不能改回 1440px 或任何固定值**，表格/卡片利用全部宽度，这是红线
- **z-index 层级自检** — 每次改布局后必检品类 flyout 是否被遮挡（flyout >= z-[100]），**已反复出问题多次**
- **重启前端必清缓存** — `rm -rf .next && pnpm dev`，不清缓存会加载旧页面
- **前端 API 必须复用 `apiFetch`** — token 在 Zustand 内存不在 localStorage，手写 fetch 取 token 会 401
- **错误展示走 Toast + i18n** — catch 块必须 `toastError(translateError(e))`，禁止直接显示 `e.message` 英文原文
- **列表页排序默认加** — 时间/数量/价格列默认加排序支持，这是 B2B 电商常识
- **浏览器 API 兼容 HTTP** — `crypto.randomUUID` 等 Secure Context API 必须写 fallback，本地和 ECS 可能是 HTTP
- **启动服务读 .env 端口** — 不要硬编码端口号，先检查 `.env.local` 和后端 `.env` 确认端口

### i18n

- **翻译必须三语** — zh / en / sw 同时覆盖，不能只做中英

### 后端开发

- **依赖分组** — 运行时 import 的包放 `[project.dependencies]`，不放 dev，否则生产镜像缺包
- **功能链路端到端验证** — 写入→异步处理→读取必须造数据跑全链路，不能只验单步语法

### 设计 & 方案

- **保持独立专业判断** — 做设计讨论的专家伙伴，不一味附和，该挑战的假设要挑战
- **设计讨论直接出可视化** — 涉及 UI/布局时直接用浏览器展示线框图，不要每次问"要不要用"
- **修 bug 必须全局审视** — 不打局部补丁：找同类、查上下游、验整条链路
- **主动设计思考** — 实现前自问：数据归属、事务边界、部署差异、保存模型、行业实践，不等用户纠正
